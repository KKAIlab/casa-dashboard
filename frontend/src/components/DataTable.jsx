    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead><tr className="border-b text-left text-gray-500">
          <th className="py-2 px-3">Name</th><th className="py-2 px-3">Genotype</th>
          <th className="py-2 px-3">Sperm</th><th className="py-2 px-3">Status</th><th className="py-2 px-3">Actions</th>
        </tr></thead>
        <tbody>{datasets.map(ds => (
          <tr key={ds.id} className={`border-b hover:bg-gray-50 cursor-pointer ${selectedId === ds.id ? 'bg-blue-50' : ''}`} onClick={() => onSelect(ds.id)}>
